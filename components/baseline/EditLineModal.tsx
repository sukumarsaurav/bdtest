...
export default function EditLineModal({ open, line, onClose }: EditLineModalProps) {
  const { updateLine, deleteLine } = useStore();
  const [form, setForm] = useState<Omit<Line, 'delta' | 'monthly'>>({
    code: '',
    route: '',
    partner: '',
    region: 'N',
  ...
  useEffect(() => {
    if (open && line) {
      setForm({
        ...
        minG: line.minG,
        pc5: line.pc5,
        dieselAtCommission: line.dieselAtCommission,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateLine(form.code, updatedLine);
    onClose();
  };

  const handleDelete = () => {
    if (line && confirm(`Are you sure you want to delete line "${line.code}"? This cannot be undone.`)) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 bg-[#444444] rounded-t-xl">
          <h2 className="text-lg font-semibold text-white">Edit Line: {line.code}</h2>
          <button
            onClick={handleDelete}
            className="px-3 py-1 text-xs font-medium text-white bg-[#FFAD00] rounded-lg hover:bg-[#FFAD00]/80 transition-colors"
          >
            Delete Line
          </button>
        </div>
